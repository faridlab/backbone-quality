...
metaphor migration generate Invoice quality
metaphor migration run
...
(`quality` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed quality          # run Rust seeders
metaphor migration generate-seeds quality  # emit SQL seed files
```
